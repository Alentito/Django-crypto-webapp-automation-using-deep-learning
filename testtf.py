...
test_data = data.iloc[501:701]  # Data after index 500 for prediction
...
# Obtain the most recent sequence from the test data
last_sequence = np.array(test_data[columns_to_normalize])[-sequence_length:]
last_sequence = np.reshape(last_sequence, (1, sequence_length, len(columns_to_normalize)))

# Predict the next value
predicted_price_normalized = model.predict(last_sequence)
predicted_price = scaler_close.inverse_transform(predicted_price_normalized)
